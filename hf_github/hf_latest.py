...
class hf_solver:
    ...
    def construct_hamiltonian(self, type):
        ...
        self.kinetic = -(self.h_bar**2 / (2 * self.mass)) * (-(5/2) * np.eye(self.Nx) +
        ...
        if self.type == "QHO":
            ...
            density = np.real(np.conj(self.initial_state) * self.initial_state)
            normalization = np.dot(self.initial_state, np.conj(self.initial_state)) * self.dx
            interaction_term = self.q * np.diag(density / normalization)
            self.potential = self.potential + interaction_term
            hamiltonian = self.kinetic + self.potential
            print(hamiltonian)
    ...
    def run_solver(self, snapshot = False, snapshot_freq = 1):
        ...
        for t in self.t_range:
            if t == 0:
                ...
                # intermediate_state = np.dot(teo_pred, self.initial_state)
                # self.construct_hamiltonian(self.type)
                # teo_final = sp.linalg.expm((-1j) * self.hamiltonian * self.dt)
                # new_state = np.dot(teo_final, self.initial_state)
                # final_state_norm = final_state_norm + 0.75 * (new_state + intermediate_state)
                next_state= np.copy(self.initial_state)
                current_correction=np.copy(self.initial_state)
                self.construct_hamiltonian(self.type)
                current_correction = sp.linalg.expm((-1j)*self.dt*self.hamiltonian)
                next_state= np.dot(next_state, current_correction)
                final_state_norm = next_state / np.sqrt(np.dot(next_state, np.conj(next_state)) * self.dx)
                

            total_energies[step] = np.real(np.vdot(final_state_norm, np.dot(self.hamiltonian, final_state_norm)))
            total_kinetics[step] = np.real(np.vdot(final_state_norm, np.dot(self.kinetic, final_state_norm)))
            total_potentials[step] = np.real(np.vdot(final_state_norm, np.dot(self.potential, final_state_norm)))
